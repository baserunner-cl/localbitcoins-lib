...
var makeHmacRequest = function (hmacOption, requestOptions, payload) {
    return Q.Promise(function (resolve, reject, notify) {
        ...
        var nonce = Date.now();
        var message = nonce + hmacOption.key + requestOptions.uri + requestOptions.uriParam;
        ...
        if (requestOptions.method == METHOD_GET) {
            https.get(apiOption, function (response) {
            ...
            https.request(apiOption, function (response) {
                response.on('data', function (data) {
                    content += data;
                });
                response.on('end', function () {
...
Connection.prototype.makeRequest = function (requestOptions, payload) {
    var self = this;

    if (self.options.useHmac === true) {
        return makeHmacRequest({key: self.options.hmacKey, secret: self.options.hmacSecret}, requestOptions, payload);
    }
